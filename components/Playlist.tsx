  ...
  return (
    <div className="flex flex-col h-full relative">
      <div className="flex-none bg-[#0a0f1e] pb-4 z-20">
        {/* Main Header Row: Vault Explorer | Plus Button | Surprise Me */}
        <div className="grid grid-cols-3 items-center mb-6 px-1">
          ...
          <div className="flex justify-center">
            <button 
              ...
              className={`w-11 h-11 flex items-center justify-center rounded-full shadow-2xl transition-all active:scale-90 transform border z-30 ${
                showAddForm 
                ? 'bg-red-600/20 text-red-500 border-red-500/40 rotate-45' 
                : 'bg-blue-600 text-white border-blue-400 hover:bg-blue-500 shadow-[0_0_20px_rgba(37,99,235,0.4)]'
              }`}
            >
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {isGeneratingRandom ? 'Syncing...' : 'Surprise Me'}
            </button>
          </div>
        </div>
          ...
          <div className="flex items-center gap-1 p-1 bg-slate-900 rounded-2xl border border-white/10 overflow-x-auto no-scrollbar shadow-lg">
            {tabs.map(tab => (
              ...
              >
                ...
                {/* Delete Category Button - Hidden for All/Vault */}
                {tab.name !== 'All' && tab.name !== 'Vault' && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); onRemoveCategory(tab.name); }}
                    className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-600 text-white flex items-center justify-center text-[7px] opacity-0 group-hover/tab:opacity-100 transition-opacity border border-black z-10"
            ...
            <div className="relative flex items-center h-full px-2 shrink-0">
              {isAddingCategory ? (
                ...
                <button 
                  ...
                  className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 text-slate-500 hover:text-blue-400 hover:bg-white/10 transition-all shadow-md"
                  title="Add Category"
                >
      ...
      <div ref={scrollContainerRef} className="flex-1 space-y-2 overflow-y-auto pr-1.5 custom-scrollbar mt-4 pb-10">
        {filteredVideos.length === 0 ? (
          ...
          >
            ...
            {/* ACTION HUD: HEART + DELETE (RESTORED) */}

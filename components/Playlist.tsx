...
const Playlist: React.FC<PlaylistProps> = ({ 
  ...
  // Helper to determine the tab's thematic color
  const getTabThematicColor = (tabName: string) => {
    if (tabName === 'All') return '#3b82f6'; // Neural Blue
    if (tabName === 'Vault') return '#ef4444'; // Security Red
  ...
  const getTabStyles = (tabName: string) => {
    ...
    if (isActive) {
      return {
        ...
        boxShadow: `0 0 12px ${color}33`, // Soft glow
        transform: 'scale(1.02)'
      };
    }
    
    return {
      color: `${color}80`, // 50% opacity text for inactive
